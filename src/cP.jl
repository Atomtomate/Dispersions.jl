...
gen_sampling(::Type{cP}, D::Int, Ns::Int) = Base.product([[(2*π/Ns) * j - π for j in 1:Ns] for Di in 1:D]...)
basis_transform(::Type{cP}, v::Tuple) = v

function reduce_KGrid(::Type{cP}, D::Int, Ns::Int, kGrid::AbstractArray)
    (Ns%2 != 0) && throw(ArgumentError("Cannot reduce simple cubic lattice with uneven number of samplng points!"))
...
conv_post(kG::KGrid{cP, D}, x::Array{T,D}) where {D, T <: Number} = reduceKArr(kG, ShiftedArrays.circshift(reverse(x), floor.(Int, gridshape(kG) ./ 2) .- 1))
...
conv_sample_post(kG::KGrid{cP, D}, x) where D = ShiftedArrays.circshift(x, floor.(Int, gridshape(kG) ./ 2) .- 1)
...
conv_post_old(kG::KGrid{cP, D}, x::Array{T,D}) where {D, T <: Number} = reduceKArr(kG, ShiftedArrays.circshift(x, floor.(Int, gridshape(kG) ./ 2) .+ 1))
