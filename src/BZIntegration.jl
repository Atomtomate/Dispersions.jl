...
function kintegrate(grid::T1, arr::Array; type::T2 = KSum()) where {T1 <: ReducedKGrid, T2 <: KSum}
    return sum(grid.kMult .* arr)/sum(grid.kMult)
end

function kintegrate(grid::T1, arr::Array; type::T2 = KSum()) where {T1 <: FullKGrid, T2 <: KSum}
