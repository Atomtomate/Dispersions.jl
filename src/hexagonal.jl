...
"""
function gen_p6m_kGrid(Nk::Int64, t::Float64)
end
...
struct FullKGrid_p6m  <: FullKGrid{p6m}
    ...
    function FullKGrid_p6m(Nk::Int, t::Float64)
        ...
        #new(Nk^2, Nk, kGrid, gen_ϵkGrid(p6m,kGrid,t),t)
    end
end
...
is fulfilled.
"""
function reduceKGrid(kG::FullKGrid{cP_p6m})
...
gen_ϵkGrid(::Type{p6m}, kGrid::GridPoints2D, t::T1) where T1 <: Number = collect(map(kᵢ -> -2*t*(cos.(0.5*(kᵢ[1] + kᵢ[2])) + cos(0.5*(kᵢ[1] - kᵢ[2])) + cos(kᵢ[1])), kGrid))
